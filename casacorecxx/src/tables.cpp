...
JLCXX_MODULE define_julia_module(jlcxx::Module &mod) {
    ...
    mod.add_bits<Slicer::LengthOrLast>("Sliceer", jlcxx::julia_type("CppEnum"));
    ...
    mod.add_type<jlcxx::Parametric<jlcxx::TypeVar<1>>>("Array")
        ...
        >([](auto wrapped) {
            ...
            wrapped.method("copy!", [](T* dst, WrappedT & src) {
                if (src.contiguousStorage()) {
                    typename WrappedT::contiter iterend(src.cend());
                    for (typename WrappedT::contiter iter = src.cbegin(); iter != iterend; ++iter) {
                        *dst = *iter;
                        dst++;
                    }
                } else {
                    typename WrappedT::iterator iterend(src.end());
                    for (typename WrappedT::iterator iter = src.begin(); iter != iterend; ++iter) {
                        *dst = *iter;
                        dst++;
                    }
                }
